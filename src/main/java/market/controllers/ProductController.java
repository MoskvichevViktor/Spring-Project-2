...
import market.models.dtos.ProductDto;
import market.error_handling.InvalidDataException;
...
public class ProductController {
    ...
    public Page<ProductDto> getAllProducts(@RequestParam(name = "p", defaultValue = "1") int page) {
        ...
        Page<ProductDto> dtoPage = new PageImpl<>(productsPage.getContent().stream().map(ProductDto::new)
    ...
    public ProductDto getOneProductById(@PathVariable Long id) {
        Product product = productService.findById(id).orElseThrow(()
                -> new ResourceNotFoundException("Product doesn't exists id: " + id));
